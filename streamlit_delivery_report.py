...
def calcola_riepilogo(gruppo):
    def calcola_blocco(df_blocco):
        ftth = df_blocco[df_blocco["TipoImpianto"] == "FTTH"]
        altro = df_blocco[df_blocco["TipoImpianto"] != "FTTH"]

        gestiti_ftth = ftth.shape[0]
        espletati_ftth = ftth[ftth["Causale"] == "COMPLWR"].shape[0]
        resa_ftth = (espletati_ftth / gestiti_ftth) * 100 if gestiti_ftth > 0 else None

        gestiti_altro = altro.shape[0]
        espletati_altro = altro[altro["Causale"] == "COMPLWR"].shape[0]
